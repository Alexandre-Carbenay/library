...
import org.adhuc.library.referencing.books.*;
...
class BooksControllerTests {
    ...
    class BookReferencing {
        ...
        @ParameterizedTest
        @MethodSource("duplicateBookProvider")
        @DisplayName("reference duplicated book")
        void referenceDuplicatedBook(String title, int position, String requestBody, String expectedPointer) throws Exception {
            when(booksReferencingService.referenceBook(any())).thenThrow(new DuplicateBookException(position, title));
            ...
            mvc.perform(post("/api/v1/books")
                    ...
                    .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
                    .andExpect(jsonPath("type", equalTo("/problems/duplicate-book")))
                    .andExpect(jsonPath("status", equalTo(409)))
                    .andExpect(jsonPath("title", equalTo("Duplicate book")))
                    .andExpect(jsonPath("detail", equalTo("Book with title already exists")))
                    .andExpect(jsonPath("errors").isArray())
                    .andExpect(jsonPath("errors", hasSize(1)))
                    .andExpect(jsonPath("errors[0].detail", equalTo("Title '%s' already exists".formatted(title))))
                    .andExpect(jsonPath("errors[0].pointer", equalTo(expectedPointer)));
        }

        private static Stream<Arguments> duplicateBookProvider() {
            ...
            return Stream.of(
                    Arguments.of(title, 0, """
                                    ...
                                    """.formatted(Books.author(), Books.language(), Books.language(), title, Books.description()),
                            "/details/0/title"
                    ),
                    Arguments.of(title, 1, """
                                    ...
                                    """.formatted(Books.author(), Books.title(), Books.description(), title, Books.description()),
                            "/details/1/title"
                    )
            );
        }
